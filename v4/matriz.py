...
def my_vector_mul():
    ...
    def device_body():
        ...
        combined_ty = np.ndarray[(N*3,), np.dtype[np.int32]]
        ...
        of_in1 = object_fifo("in1", ShimTile, ComputeTile2, buffer_depth, combined_ty)
        of_in2 = object_fifo("in2", ShimTile, ComputeTile2, buffer_depth, tensor_ty)
        of_out = object_fifo("out", ComputeTile2, ShimTile, buffer_depth, tensor_ty)

        # Set up compute tiles
        matvec = external_func(
            f"matvec_i32_i32",
            inputs=[combined_ty, tensor_ty , tensor_ty],
        )
        # Compute tile 2
        @core(ComputeTile2)
        def core_body():
            ...
            for _ in range_(sys.maxsize):
                ...
                for _ in range_(N_div_n):
                    ...
                    matvec(elem_in1, elem_in2, elem_out)
                    of_in1.release(ObjectFifoPort.Consume, 1)
                    of_in2.release(ObjectFifoPort.Consume, 1)
                    of_out.release(ObjectFifoPort.Produce, 1)

        # To/from AIE-array data movement
        @runtime_sequence(combined_ty, tensor_ty, tensor_ty)
        def sequence(A, B, C):
